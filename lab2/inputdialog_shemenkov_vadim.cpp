...
InputDialog_Shemenkov_Vadim::InputDialog_Shemenkov_Vadim(QWidget* pwgt):QDialog(pwgt){
    ...
    connect(pcmdOk, SIGNAL(cliced()),SLOT(accept()));
...
QString InputDialog_Shemenkov_Vadim::firstName() const{
    return m_ptxtFirstName->text();
}
QSting InputDialog_Shemenkov_Vadim::lastName() const {
